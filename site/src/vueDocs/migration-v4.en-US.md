This document will help you upgrade from trina-design-vue `3.x` version to trina-design-vue `4.x` version. If you are using `2.x` or older version, please refer to the previous [upgrade document](/docs/vue/migration-v3) to 3.x.
...
- Remove less, adopt CSS-in-JS, for better support of dynamic themes.
  ...
  - Css files are no longer included in package. Since CSS-in-JS supports importing on demand, the original `trina-design-vue/dist/antd.css` has also been abandoned. If you need to reset some basic styles, please import `trina-design-vue/dist/reset.css`.
  - If you need to reset the style of the component, but you don't want to introduce `trina-design-vue/dist/reset.css` to pollute the global style, You can try using the [App](/components/app) in the outermost layer to solve the problem that native elements do not have antd specification style.
- Remove css variables and dynamic theme built on top of them.
- LocaleProvider has been deprecated in 3.x (use `<ConfigProvider locale />` instead), we removed the related folder `trina-design-vue/es/locale-provider` and `trina-design-vue/lib/locale-provider` in 4.x.
...
```bash
npm install --save trina-design-vue@4
```

### less migration

If you using trina-design-vue less variables, you can use compatible package to covert it into v3 less variables and use less-loader to inject them:

```js
const { theme } = require('trina-design-vue/lib');
const convertLegacyToken = require('trina-design-vue/lib/theme/convertLegacyToken');
...
Ant then remove trina-design-vue less reference in your less file:

```diff
// Your less file
--  @import (reference) '~trina-design-vue/es/style/themes/index';
or
--  @import '~trina-design-vue/es/style/some-other-less-file-ref';
```
...
```diff
"plugins": [
-- ["import", { "libraryName": "trina-design-vue", "libraryDirectory": "lib"}, "trina-design-vue"],
]
```

### Legacy browser support

Trina Design Vue v4 using `:where` css selector to reduce CSS-in-JS hash priority. You can use `StyleProvider` to cancel this function. Please ref [Compatible adjustment](/docs/vue/compatible-style).

## Encounter problems

If you encounter problems during the upgrade, please go to [GitHub issues](https://github.com/kaoniqiwa/trina-design-vue/issues) for feedback. We will respond and improve this document as soon as possible.
